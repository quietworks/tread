...
export class ArticleList extends BoxRenderable {
	...
	private updateScrollOffset(): void {
		...
		if (this.selectedIndex < this.scrollOffset + padding) {
			...
			this.selectedIndex >=
			this.scrollOffset + this.visibleCount - padding
		) {
	...
	private renderItems(): void {
		...
		for (let i = 0; i < visibleArticles.length; i++) {
			...
			const padding = " ".repeat(
				Math.max(
					0,
					contentWidth - readIndicator.length - title.length - dateWidth,
				),
			);
		...
		if (this.articles.length > this.visibleCount) {
			const hasMore =
				this.scrollOffset + this.visibleCount < this.articles.length;
