...
export class FeedList extends BoxRenderable {
  ...
  private renderItems(): void {
    ...
    for (let i = 0; i < this.items.length; i++) {
      ...
      const maxNameWidth = contentWidth - indicator.length - unreadStr.length - 1;
      const name = item.feed.name.length > maxNameWidth
        ? item.feed.name.slice(0, maxNameWidth - 1) + "\u2026"
        : item.feed.name;

      const text = new TextRenderable(this._ctx, {
        content: `${indicator}${name}${unreadStr}`,
        fg: isSelected && this._isFocused ? colors.fg : (item.unreadCount > 0 ? colors.fg : colors.fgDim),
        width: contentWidth,
        height: 1,
      });
